...
import {
  ...
  SubscriptionPausedMetadata$inboundSchema,
} from "./subscriptionpausedmetadata.js";

/**
 * An event created by Polar when a subscription is paused.
 */
export type SubscriptionPausedEvent = {
  /**
...
/** @internal */
export const SubscriptionPausedEvent$inboundSchema: z.ZodMiniType<
...
> = z.pipe(
  z.object({
    ...
    name: z.literal("subscription.paused"),
    metadata: SubscriptionPausedMetadata$inboundSchema,
  }),
  z.transform((v) => {
    return remap$(v, {
...
export function subscriptionPausedEventFromJSON(
  jsonString: string,
): SafeParseResult<SubscriptionPausedEvent, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SubscriptionPausedEvent$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SubscriptionPausedEvent' from JSON`,
  );
}
