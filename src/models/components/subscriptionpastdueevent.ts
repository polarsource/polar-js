...
import {
  ...
  SubscriptionPastDueMetadata$inboundSchema,
} from "./subscriptionpastduemetadata.js";

/**
 * An event created by Polar when a subscription becomes past due.
 */
export type SubscriptionPastDueEvent = {
  /**
...
/** @internal */
export const SubscriptionPastDueEvent$inboundSchema: z.ZodMiniType<
...
> = z.pipe(
  z.object({
    ...
    name: z.literal("subscription.past_due"),
    metadata: SubscriptionPastDueMetadata$inboundSchema,
  }),
  z.transform((v) => {
    return remap$(v, {
...
export function subscriptionPastDueEventFromJSON(
  jsonString: string,
): SafeParseResult<SubscriptionPastDueEvent, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SubscriptionPastDueEvent$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SubscriptionPastDueEvent' from JSON`,
  );
}
