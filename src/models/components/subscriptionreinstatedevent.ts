...
import {
  ...
  SubscriptionReinstatedMetadata$inboundSchema,
} from "./subscriptionreinstatedmetadata.js";

/**
 * An event created by Polar when a canceled subscription is reinstated.
 */
export type SubscriptionReinstatedEvent = {
  /**
...
/** @internal */
export const SubscriptionReinstatedEvent$inboundSchema: z.ZodMiniType<
...
> = z.pipe(
  z.object({
    ...
    name: z.literal("subscription.reinstated"),
    metadata: SubscriptionReinstatedMetadata$inboundSchema,
  }),
  z.transform((v) => {
    return remap$(v, {
...
export function subscriptionReinstatedEventFromJSON(
  jsonString: string,
): SafeParseResult<SubscriptionReinstatedEvent, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SubscriptionReinstatedEvent$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SubscriptionReinstatedEvent' from JSON`,
  );
}
