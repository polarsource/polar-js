...
import {
  ...
  SubscriptionResumedMetadata$inboundSchema,
} from "./subscriptionresumedmetadata.js";
...
/** @internal */
export const SubscriptionResumedEvent$inboundSchema: z.ZodMiniType<
...
> = z.pipe(
  z.object({
    ...
    name: z.literal("subscription.resumed"),
    metadata: SubscriptionResumedMetadata$inboundSchema,
  }),
  z.transform((v) => {
    return remap$(v, {
...
export function subscriptionResumedEventFromJSON(
  jsonString: string,
): SafeParseResult<SubscriptionResumedEvent, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SubscriptionResumedEvent$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SubscriptionResumedEvent' from JSON`,
  );
}
