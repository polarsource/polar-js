...
import {
  ...
  SubscriptionUpdateClearedMetadata$inboundSchema,
} from "./subscriptionupdateclearedmetadata.js";

/**
 * An event created by Polar when a pending subscription update is cleared without being applied.
 */
export type SubscriptionUpdateClearedEvent = {
  /**
...
/** @internal */
export const SubscriptionUpdateClearedEvent$inboundSchema: z.ZodMiniType<
...
> = z.pipe(
  z.object({
    ...
    name: z.literal("subscription.update_cleared"),
    metadata: SubscriptionUpdateClearedMetadata$inboundSchema,
  }),
  z.transform((v) => {
    return remap$(v, {
...
export function subscriptionUpdateClearedEventFromJSON(
  jsonString: string,
): SafeParseResult<SubscriptionUpdateClearedEvent, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SubscriptionUpdateClearedEvent$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SubscriptionUpdateClearedEvent' from JSON`,
  );
}
