/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { remap as remap$ } from "../../lib/primitives.js";
import * as z from "zod";

export type Assignee = {
    id: number;
    login: string;
    htmlUrl: string;
    avatarUrl: string;
};

/** @internal */
export const Assignee$inboundSchema: z.ZodType<Assignee, z.ZodTypeDef, unknown> = z
    .object({
        id: z.number().int(),
        login: z.string(),
        html_url: z.string(),
        avatar_url: z.string(),
    })
    .transform((v) => {
        return remap$(v, {
            html_url: "htmlUrl",
            avatar_url: "avatarUrl",
        });
    });

/** @internal */
export type Assignee$Outbound = {
    id: number;
    login: string;
    html_url: string;
    avatar_url: string;
};

/** @internal */
export const Assignee$outboundSchema: z.ZodType<Assignee$Outbound, z.ZodTypeDef, Assignee> = z
    .object({
        id: z.number().int(),
        login: z.string(),
        htmlUrl: z.string(),
        avatarUrl: z.string(),
    })
    .transform((v) => {
        return remap$(v, {
            htmlUrl: "html_url",
            avatarUrl: "avatar_url",
        });
    });

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace Assignee$ {
    /** @deprecated use `Assignee$inboundSchema` instead. */
    export const inboundSchema = Assignee$inboundSchema;
    /** @deprecated use `Assignee$outboundSchema` instead. */
    export const outboundSchema = Assignee$outboundSchema;
    /** @deprecated use `Assignee$Outbound` instead. */
    export type Outbound = Assignee$Outbound;
}
