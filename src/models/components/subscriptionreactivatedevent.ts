...
import {
  ...
  SubscriptionReactivatedMetadata$inboundSchema,
} from "./subscriptionreactivatedmetadata.js";

/**
 * An event created by Polar when a past due subscription is recovered.
 */
export type SubscriptionReactivatedEvent = {
  /**
...
/** @internal */
export const SubscriptionReactivatedEvent$inboundSchema: z.ZodMiniType<
...
> = z.pipe(
  z.object({
    ...
    name: z.literal("subscription.reactivated"),
    metadata: SubscriptionReactivatedMetadata$inboundSchema,
  }),
  z.transform((v) => {
    return remap$(v, {
...
export function subscriptionReactivatedEventFromJSON(
  jsonString: string,
): SafeParseResult<SubscriptionReactivatedEvent, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SubscriptionReactivatedEvent$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SubscriptionReactivatedEvent' from JSON`,
  );
}
