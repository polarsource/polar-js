...
import {
  ...
  OrderUnvoidedMetadata$inboundSchema,
} from "./orderunvoidedmetadata.js";

/**
 * An event created by Polar when an order is unvoided.
 */
export type OrderUnvoidedEvent = {
  /**
...
/** @internal */
export const OrderUnvoidedEvent$inboundSchema: z.ZodMiniType<
  OrderUnvoidedEvent,
  unknown
> = z.pipe(
  z.object({
    ...
    name: z.literal("order.unvoided"),
    metadata: OrderUnvoidedMetadata$inboundSchema,
  }),
  z.transform((v) => {
    return remap$(v, {
...
export function orderUnvoidedEventFromJSON(
  jsonString: string,
): SafeParseResult<OrderUnvoidedEvent, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => OrderUnvoidedEvent$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'OrderUnvoidedEvent' from JSON`,
  );
}
