 ...
 */
export async function licenseKeysUpdate(
  client$: PolarCore,
  request: operations.LicenseKeysUpdateRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.LicenseKeyRead,
    | errors.Unauthorized
...
> {
  ...
  const parsed$ = schemas$.safeParse(
    input$,
    (value$) =>
      operations.LicenseKeysUpdateRequest$outboundSchema.parse(value$),
  ...
  const body$ = encodeJSON$("body", payload$.LicenseKeyUpdate, {
    explode: true,
  });
  ...
  const [result$] = await m$.match<
    ...
    | ConnectionError
  >(
    m$.json(200, components.LicenseKeyRead$inboundSchema),
